...
const Admin = () => {
  ...
  const handleDelete = async (id) => {
    setShowDialog(false);
    const res = await FetchApi(token, language, "deleteAdmin", {}, id);
    if (res.success) {
  ...
  return (
    <>
      ...
      <PageTitle
        title={t("admin.title")}
        handleNavigate={() => navigate("/app/admin/create")}
        titleButton={t("all.create")}
      />
      <TableFrame
        loading={loading}
        notFoundText={t("admin.notFound")}
        firstItem={!!rows[0]}
      >
        <TableHead>
          <TableRow
          ...
          >
            <TableCell align="center">{t("admin.firstname")}</TableCell>
            <TableCell align="center">{t("admin.lastname")}</TableCell>
            <TableCell align="center">{t("admin.username")}</TableCell>
          ...
          {rows.map((row, index) => (
            <TableRow key={index}>
              ...
              <TableCell align="center">
                <IconButton
                  onClick={() => navigate(`/app/admin/edit/${row.id}`)}
                >
                  <EditIcon />
                </IconButton>
